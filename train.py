...
def train(args):
    ...
    for episode in train_env.get_episodes():
        ...
        logger.info('success_rate at iter %d: %f' % (agent.update_steps, np.sum(rewards) / batch_size))
...
def test(agent, args, writer, test_env, mode='dev', print_paths=False):
    ...
    writer.add_scalar('Hits10', all_final_reward_10, agent.update_steps)
    writer.add_scalar('Hits1', all_final_reward_1, agent.update_steps)
    writer.add_scalar('Hits3', all_final_reward_3, agent.update_steps)
    writer.add_scalar('Hits5', all_final_reward_5, agent.update_steps)
    writer.add_scalar('Hits20', all_final_reward_20, agent.update_steps)
    writer.add_scalar('AUC', auc, agent.update_steps)
